...
class PhotosTestCase(TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        super(PhotosTestCase, cls).setUpClass()
    ...
    def _call_cli(self, cli_args, stdin=None):
        full_command = [PYTHON_BIN, self.cli_path, 'photos'] + cli_args
    ...
    def test_cli_fails_without_output_path(self):
        rv, so, se = self._call_cli([])

        self.assertEqual(rv, 2)
        self.assertIn(b'{run,authenticate}', se)
